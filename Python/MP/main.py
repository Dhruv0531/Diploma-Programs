...
mysql = mq.connect(host="localhost", user="root",
                   password="", database="pythonproject")
...
def insertData(full_name, age, gender, teacher_id, branch, dob, master):
    ...
    if (flag):
        ...
        try:
            mycursor.execute("INSERT INTO teacher (name, age, gender, teacher_id, branch, dateOfBirth) VALUES (%s, %s, %s, %s, %s, %s)",
                             (full_name, age, gender, teacher_id, branch, dob))
            mysql.commit()
...
def dataManipulate(id, master, self):
    ...
    if (flag):
        update = tk.Button(self, text="Update", width=10, font=(
            "Arial", 15), command=lambda: updateData(id, master, self))
        update.grid(row=3, column=1, padx=(10, 5), pady=10)

        delete = tk.Button(self, text="Delete", width=10, font=(
            "Arial", 15), command=lambda: deleteData(id, master))
...
def updateData(id, master, self):
    ...
    try:
        ...
        master.switch_frame(UpdatePage)

    except mq.Error as e:
        print("Error", e)


def update(full_name, age, gender, teacher_id, branch, dob, master):
    try:
        mycursor.execute("INSERT INTO teacher (name, age, gender, teacher_id, branch, dateOfBirth) VALUES (%s, %s, %s, %s, %s, %s)",
                         (full_name, age, gender, teacher_id, branch, dob))
        mysql.commit()
...
def getData(branch, gender):
    ...
    if branch == "All Branches" and gender == "All Teachers":
        ...
        mycursor.execute(
            "SELECT * from teacher Where branch=%s and gender = %s", (branch, gender))
    result = mycursor.fetchall()
    return result
...
def displayData(branch, gender, master, self):
    ...
    for i in range(len(parameter)):
        e = Entry(self, width=20, fg='black', font=(
    ...
    for i in range(len(teacherList)):
        ...
        for j in range(len(teacherList[i])):
            f.append(Entry(self, width=20, fg='blue', font=(
                'Arial', 16, 'bold'), justify='center'))
    ...
    clearbtn = tk.Button(self, text="Clear", width=9, font=(
        "Ariel", 12), command=lambda: clear(g))
...
class SampleApp(tk.Tk):
    ...
    def switch_frame(self, frame_class, id=1):
...
class StartPage(tk.Frame):
    def __init__(self, master):
        ...
        button = tk.Button(self, text="Enter Teacher Data", height=5, width=37, font=(
            "Ariel", 15), command=lambda: master.switch_frame(PageOne))
        button.place(relx=0.5, rely=0.2, anchor=tk.CENTER)

        button2 = tk.Button(self, text="View Teacher Data", height=5, width=37, font=(
            "Ariel", 15), command=lambda: master.switch_frame(PageTwo))
        button2.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

        button3 = tk.Button(self, text="Delete/Update Teacher Data", height=5, width=37,
                            font=("Ariel", 15), command=lambda: master.switch_frame(PageThree))
...
class PageOne(tk.Frame):
    def __init__(self, master):
        ...
        branch_list = ["Computer Engineering", "Mechanical Engineering",
                       "Electrical Engineering", "Civil Engineering", "Autombile Enginnering"]
        value_inside = tk.StringVar(self)
        value_inside.set("Select an option")

        label_name = tk.Label(
            self, text="Teacher Registration", font=("Arial", 18))
        ...
        entry_gender_m = tk.Radiobutton(self, text="Male", font=(
            "Arial", 12), variable=x, value="MALE")
        entry_gender_m.grid(row=3, column=1)
        entry_gender_f = tk.Radiobutton(self, text="Female", font=(
            "Arial", 12), variable=x, value="FEMALE")
        ...
        entry_dob = Calendar(self, selectmode='day',
                             year=2020, month=5, day=22)
        entry_dob.grid(row=6, column=1, padx=10, pady=5)

        button_back = tk.Button(self, text="Back", font=(
            "Arial", 15), width=10, command=lambda: master.switch_frame(StartPage))
...
class PageTwo(tk.Frame):
    def __init__(self, master):
        ...
        branch_list = ["Computer Engineering", "Mechanical Engineering",
                       "Electrical Engineering", "All Branches", "Civil Engineering", "Autombile Enginnering"]
        ...
        button_back = tk.Button(self, text="Back", width=9, font=(
            "Arial", 12), command=lambda: master.switch_frame(StartPage))
        button_back.grid(row=1, column=0, padx=(10, 5), pady=10)

        submit_button = tk.Button(self, text="Display Data", width=10, font=(
            "Arial", 12), command=lambda: displayData(value_inside1.get(), value_inside2.get(), master, self))
...
class PageThree(tk.Frame):
    def __init__(self, master):
        ...
        title = tk.Label(
            self, text="Delete/Update Teacher Data", font=("Arial", 18))
        title.grid(row=0, column=1, columnspan=3, pady=10)

        label1 = tk.Label(self, text="Enter Teacher ID  : ",
                          font=("Arial", 12))
        ...
        button_back = tk.Button(self, text="Back", width=10, font=(
            "Arial", 15), command=lambda: master.switch_frame(StartPage))
        button_back.grid(row=2, column=1, padx=(10, 5), pady=10)

        submit_button = tk.Button(self, text="Submit", width=10, font=(
            "Arial", 15), command=lambda: dataManipulate(deleteId.get(), master, self))
